...
- **`tsconfig.build.json` sin `exclude` de `frontend`**: el compilador de NestJS procesaba archivos JSX del frontend causando errores de compilación.
- **Variables de entorno con path relativo a `__dirname`**: fallaba según el working directory al momento de ejecución.
- **Typos en mensajes de error**: `wronCredentials` e `invlidToken` causaban referencias rotas en el código.
...
┌─────────────────────────────────────────────────────┐
│                    Frontend React                    │
│  WebSocket (tiempo real) + Polling fallback (60s)   │
└──────────────┬──────────────────────────────────────┘
               │ HTTP REST + WebSocket
┌──────────────▼──────────────────────────────────────┐
│                   NestJS Backend                     │
│                                                      │
│  ┌──────────┐  ┌──────────┐  ┌───────────────────┐  │
│  │  Product  │  │Inventory │  │  WebSocket Gateway│  │
│  │  Module   │  │  Module  │  │   /events (WS)    │  │
│  └─────┬─────┘  └────┬─────┘  └─────────▲─────────┘  │
│        │             │                  │            │
│        └──────┬──────┘                  │            │
│               │ EventEmitter2           │            │
...
├── backend/
│   ├── src/
│   │   ├── api/              # Módulos: auth, user, role, product, inventory
│   │   ├── common/           # Helpers, interceptors, decoradores
│   │   ├── config/           # Variables de entorno
│   │   ├── database/         # Entidades, migraciones, seeders
│   │   ├── events/           # domain-events.ts — clases de eventos
│   │   └── websocket/        # Gateway de Socket.io
│   ├── src/common/envs/      # Archivos de entorno por ambiente
│   └── docker-compose.yml    # PostgreSQL para desarrollo local
└── frontend/
    └── src/
        ├── api/              # client.ts — axios con interceptor de token
        ├── hooks/            # usePolling.ts, useSocket.ts
        ├── components/       # ProductCard, InventoryTable
...
**Backend**: NestJS v9 · TypeORM · PostgreSQL · @nestjs/event-emitter · @nestjs/websockets · Socket.io · JWT · Swagger

**Frontend**: React · TypeScript · socket.io-client · axios

**Deploy**: Railway (backend + frontend + PostgreSQL gestionada)
